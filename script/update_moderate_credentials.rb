#!/usr/bin/env ruby
# Script to update Moderate persona with Twitter API credentials

persona = Persona.find_by(slug: 'moderate')

if persona.nil?
  puts "❌ Moderate persona not found!"
  exit 1
end

persona.update!(
  twitter_api_key: ENV['X_MODERATE_API_KEY'],
  twitter_api_secret: ENV['X_MODERATE_API_SECRET'],
  twitter_access_token: ENV['X_MODERATE_ACCESS_TOKEN'],
  twitter_access_token_secret: ENV['X_MODERATE_ACCESS_TOKEN_SECRET'],
  twitter_enabled: true,
  twitter_handle: 'TModerate'
)

puts "✅ Moderate persona updated successfully!"
puts ""
